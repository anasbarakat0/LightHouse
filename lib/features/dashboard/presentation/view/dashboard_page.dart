...
import 'package:lighthouse/core/utils/file_download/file_download_saver.dart';
...
import 'package:lighthouse/features/dashboard/presentation/widget/dashboard_customer_reports.dart';
...
class _DashboardPageState extends State<DashboardPage> {
      ...
      providers: [
            ...
            getDashboardSummaryUsecase: GetDashboardSummaryUsecase(
              getDashboardSummaryRepo: GetDashboardSummaryRepo(
                getDashboardSummaryService:
                    GetDashboardSummaryService(dio: Dio()),
            ...
            getDashboardRevenueChartUsecase: GetDashboardRevenueChartUsecase(
              getDashboardRevenueChartRepo: GetDashboardRevenueChartRepo(
                getDashboardRevenueChartService:
                    GetDashboardRevenueChartService(dio: Dio()),
                networkConnection: NetworkConnection.createDefault(),
              ),
            ),
          ),
        ...
        BlocProvider(
          create: (context) => GetDashboardRecentSessionsBloc(
            getDashboardRecentSessionsUsecase:
                GetDashboardRecentSessionsUsecase(
              getDashboardRecentSessionsRepo: GetDashboardRecentSessionsRepo(
                getDashboardRecentSessionsService:
                    GetDashboardRecentSessionsService(dio: Dio()),
                networkConnection: NetworkConnection.createDefault(),
              ),
            ),
          ),
            ...
            getDashboardAlertsUsecase: GetDashboardAlertsUsecase(
              getDashboardAlertsRepo: GetDashboardAlertsRepo(
                getDashboardAlertsService:
                    GetDashboardAlertsService(dio: Dio()),
                networkConnection: NetworkConnection.createDefault(),
              ),
            ),
          ),
        ...
        BlocProvider(
          create: (context) => CustomerReportsBloc(
            customerReportsUsecase: CustomerReportsUsecase(
              customerReportsRepo: CustomerReportsRepo(
                customerReportsService: CustomerReportsService(dio: Dio()),
                networkConnection: NetworkConnection.createDefault(),
              ),
            ),
          ),
        ...
        builder: (builderContext) {
          if (!_hasRequestedInitialData) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              ...
              builderContext
                  .read<GetDashboardAlertsBloc>()
                  .add(GetDashboardAlerts());
              builderContext.read<CustomerReportsBloc>().add(
                    GetCustomerBirthdayReminders(daysBefore: 7),
                  );
            });
          }

          return BlocListener<CustomerReportsBloc, CustomerReportsState>(
            listenWhen: (previous, current) =>
                previous.csvExport != current.csvExport ||
                previous.errorMessage != current.errorMessage,
            listener: _handleCustomerReportsState,
            child: Scaffold(
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverList(
                      delegate: SliverChildListDelegate([
                        ...
                        DashboardCustomerReports(isMobile: isMobile),
                        ...
                        DashboardRevenueChart(
                          ...
                          onPeriodChanged: (period) {
                            ...
                            builderContext
                                .read<GetDashboardRevenueChartBloc>()
                                .add(
                                  GetDashboardRevenueChart(period: apiPeriod),
                                );
                          },
  ...
  void _handleCustomerReportsState(
    ...
    final csvExport = state.csvExport;
    if (csvExport != null) {
      unawaited(_saveCsvExport(context, state));
    }
  }

  Future<void> _saveCsvExport(
    BuildContext context,
    CustomerReportsState state,
  ) async {
    final csvExport = state.csvExport;
    if (csvExport == null) return;

    try {
      final savedPath = await saveCsvFile(
        bytes: csvExport.bytes,
        fileName: csvExport.fileName,
      );
      if (!mounted || !context.mounted) return;

      final message = savedPath == null
          ? 'CSV download started'.tr()
          : '${'CSV saved to'.tr()} $savedPath';
