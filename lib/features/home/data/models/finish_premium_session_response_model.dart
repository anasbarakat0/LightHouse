...
class SessionInvoice {
  ...
  factory SessionInvoice.fromMap(Map<String, dynamic> map) {
    ...
    if (map['hoursAmount'] != null) {
      ...
      } else if (map['hoursAmount'] is String) {
        try {
          hoursAmountValue = double.parse(map['hoursAmount'] as String);
        } catch (e) {
          hoursAmountValue = 0.0;
        }
      } else {
        try {
          final stringValue = map['hoursAmount'].toString().trim();
          if (stringValue.isNotEmpty) {
            hoursAmountValue = double.parse(stringValue);
          }
        } catch (e) {
          hoursAmountValue = 0.0;
        }
